...
// Ниже: гости, средний чек, avg по дням недели, события.

import { useState } from 'react';
import { BarChart2, Users, FileText, AlertTriangle, CheckCircle, X } from 'lucide-react';
        ...
        {rows.map((row, i) => {
          ...
          return (
            <div key={i} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2, minWidth: 38 }}>
              ...
              {row.avgFact != null && (
                <span style={{ fontSize: 9, color: 'var(--mt)', opacity: .65 }}>{kRub(row.avgFact)}</span>
              )}
            </div>
          );
        })}
...
export function MonthAnalytics({ revenue, events, ym, ds, isManager, monthPlan = {}, onSetMonthPlan }) {
  ...
  // ── План к дате (план за прошедшие дни) ──
  ...
  // Приближённый итог прошлого года за сопоставимое число дней
  ...
  // Прогноз гостей на месяц (если есть данные)
  ...
  return (
    <div className="sec" style={{ paddingTop: 14 }}>
      ...
      <div style={{ background: 'var(--sf)', borderRadius: 12, padding: '14px 16px',
        ...
        {/* Большое число факта + светофорный % */}
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, color: 'var(--mt)', marginTop: 4, flexWrap: 'wrap', gap: 4 }}>
          ...
          {isCurMonth && planToDate > 0 ? (
            ...
            <span style={{ opacity: .6 }}>{elapsed} из {daysInMonth} {plural(daysInMonth,'день','дня','дней')}</span>
          )}
          {/* Справа: осталось до цели */}
        ...
        {/* Конверсия дней (если есть дни с планом) */}
        {daysWithBoth.length > 0 && (
          <div style={{ fontSize: 10, color: 'var(--mt)', opacity: .6, marginTop: 3 }}>
            {daysHitPlan.length} из {daysWithBoth.length} {plural(daysWithBoth.length,'день','дня','дней')} выполнили план
          </div>
        )}

        {/* ── Прогноз (только текущий незакрытый месяц) ── */}
        ...
        {lyDelta != null && (
          <div style={{ marginTop: 10, paddingTop: 10, borderTop: '1px solid var(--bd)',
            ...
            <span style={{
              color: lyDelta > 0 ? '#8bc47a' : '#e85535',
              fontWeight: 700,
              filter: 'brightness(1.3)', // чуть ярче чем opacity .55
            }}>
              {lyDelta > 0 ? '+' : ''}{lyDelta}%
            </span>
            <span style={{ opacity: .7, fontSize: 10 }}>({lyDays.length} дн. с данными)</span>
          </div>
        )}
      </div>
      ...
      {totalGuests > 0 ? (
        <div style={{ display: 'flex', gap: 8, marginBottom: 10 }}>
          {/* Гости */}
          <div style={{ flex: 1, background: 'var(--sf)', borderRadius: 11, padding: '11px 14px', border: '1px solid var(--bd)' }}>
            <div style={{ fontSize: 10, color: 'var(--mt)', textTransform: 'uppercase', letterSpacing: '.06em', fontWeight: 600, marginBottom: 3, display: 'flex', alignItems: 'center', gap: 4 }}>
              <Users size={11}/>Гостей
            </div>
            <div style={{ fontSize: 20, fontWeight: 700, letterSpacing: -0.3 }}>{fmt(totalGuests)}</div>
            <div style={{ fontSize: 10, color: 'var(--mt)', opacity: .55, marginTop: 4 }}>
              {avgGpD != null && `≈ ${fmt(avgGpD)}/день · ${guestDays.length} дн.`}
              {projGuests != null && <span style={{ display: 'block', marginTop: 1 }}>прогноз: ~{fmt(projGuests)}</span>}
            </div>
          </div>
          {/* Средний чек */}
          {avgCheck != null && (
            <div style={{ flex: 1, background: 'var(--sf)', borderRadius: 11, padding: '11px 14px', border: '1px solid var(--bd)' }}>
              <div style={{ fontSize: 10, color: 'var(--mt)', textTransform: 'uppercase', letterSpacing: '.06em', fontWeight: 600, marginBottom: 3, display: 'flex', alignItems: 'center', gap: 4 }}>
                <FileText size={11}/>Средний чек
              </div>
              <div style={{ fontSize: 20, fontWeight: 700, letterSpacing: -0.3 }}>{fmt(avgCheck)} ₽</div>
              <div style={{ fontSize: 10, color: 'var(--mt)', opacity: .55, marginTop: 4 }}>
                за {guestDays.length} {plural(guestDays.length,'день','дня','дней')}
              </div>
            </div>
          )}
        </div>
      ) : (
        <div style={{ fontSize: 11, color: 'var(--mt)', opacity: .45, fontStyle: 'italic', marginBottom: 10 }}>
          Данные гостей — после синхронизации с iiko
        </div>
      )}
