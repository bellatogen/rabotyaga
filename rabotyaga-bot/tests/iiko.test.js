...
// Регрессия: отменённые заказы (fact=0) сохраняют положительный GuestNum в iiko OLAP.
// Гостей считаем только по заказам с ненулевой выручкой — иначе фантомные гости (см. commit 69bc3bd).
await test('отменённый заказ (fact=0) не прибавляет гостей', async () => {
  global.fetch = seqFetch(
    AUTH_OK,
    olapRows(
      { 'OpenDate.Typed': '2026-06-23', OrderNum: '0001', DishDiscountSumInt: 0,      GuestNum: 3  }, // отменён
      { 'OpenDate.Typed': '2026-06-23', OrderNum: '0002', DishDiscountSumInt: 150000, GuestNum: 80 }, // реальный
    ),
    OLAP_EMPTY, // YoY
  );
  const iiko = loadIiko();
  const r = await iiko.getDayRevenue('2026-06-23', mkData(), noop);
  assert.strictEqual(r.fact,   150000, 'выручка = сумма всех заказов (включая нулевые)');
  assert.strictEqual(r.guests, 80,     'гости только реального заказа, фантомные 3 исключены');
});
...
// ── Регрессия: GuestNum-инфляция от отменённых заказов (commit 69bc3bd) ───────

await test('cancelled orders (fact=0, guests>0) не учитываются в гостях', async () => {
  const fakeData = mkData();
  global.fetch = seqFetch(AUTH_OK, olapRows(
    { 'OpenDate.Typed': '2026-06-10', OrderNum: '0001', DishDiscountSumInt: 100000, GuestNum: 50 }, // реальный
    { 'OpenDate.Typed': '2026-06-10', OrderNum: '0002', DishDiscountSumInt: 120000, GuestNum: 60 }, // реальный
    { 'OpenDate.Typed': '2026-06-10', OrderNum: '0003', DishDiscountSumInt: 0,      GuestNum: 5  }, // отменён
  ));
  const iiko = loadIiko();
  const r = await iiko.syncRevenue(fakeData, noop);
  assert.strictEqual(r.updated, 1);
  const kv = JSON.parse(fakeData.kv['revenue:v1']);
  assert.strictEqual(kv['2026-06-10'].fact,     220000, 'выручка = сумма всех (отменённый даёт 0)');
  assert.strictEqual(kv['2026-06-10'].guests,   110,    'гости 50+60, фантомные 5 исключены');
  assert.strictEqual(kv['2026-06-10'].avgCheck, 2000,   '220000/110');
});

await test('несколько OrderNum-строк на дату аккумулируются правильно', async () => {
  const fakeData = mkData();
  global.fetch = seqFetch(AUTH_OK, olapRows(
    { 'OpenDate.Typed': '2026-06-11', OrderNum: '0001', DishDiscountSumInt: 30000, GuestNum: 10 },
    { 'OpenDate.Typed': '2026-06-11', OrderNum: '0002', DishDiscountSumInt: 40000, GuestNum: 15 },
    { 'OpenDate.Typed': '2026-06-11', OrderNum: '0003', DishDiscountSumInt: 50000, GuestNum: 25 },
  ));
  const iiko = loadIiko();
  const r = await iiko.syncRevenue(fakeData, noop);
  assert.strictEqual(r.updated, 1);
  const kv = JSON.parse(fakeData.kv['revenue:v1']);
  assert.strictEqual(kv['2026-06-11'].fact,     120000, 'fact = 30000+40000+50000');
  assert.strictEqual(kv['2026-06-11'].guests,   50,     'guests = 10+15+25');
  assert.strictEqual(kv['2026-06-11'].avgCheck, 2400,   '120000/50');
});

await test('смешанные даты с отменёнными заказами — только факт-дни в KV', async () => {
  const fakeData = mkData();
  global.fetch = seqFetch(AUTH_OK, olapRows(
    { 'OpenDate.Typed': '2026-06-12', OrderNum: '0001', DishDiscountSumInt: 100000, GuestNum: 50 }, // реальный
    { 'OpenDate.Typed': '2026-06-12', OrderNum: '0002', DishDiscountSumInt: 0,      GuestNum: 7  }, // отменён
    { 'OpenDate.Typed': '2026-06-13', OrderNum: '0003', DishDiscountSumInt: 80000,  GuestNum: 40 }, // реальный
  ));
  const iiko = loadIiko();
  const r = await iiko.syncRevenue(fakeData, noop);
  assert.strictEqual(r.updated, 2);
  const kv = JSON.parse(fakeData.kv['revenue:v1']);
  assert.strictEqual(kv['2026-06-12'].fact,   100000);
  assert.strictEqual(kv['2026-06-12'].guests, 50, 'фантомные 7 исключены из 12-го');
  assert.strictEqual(kv['2026-06-13'].fact,   80000);
  assert.strictEqual(kv['2026-06-13'].guests, 40);
});
...
await test('кэш актуальный (<20ч) → fetch не вызывается', async () => {
  ...
  const fakeData = { kv: { 'basket:pairs:v4': JSON.stringify({
    v: 4,
...
await test('basket: результат сохраняется в KV', async () => {
  ...
  assert.ok(saved, 'saveData не вызван');
  assert.ok(fakeData.kv['basket:pairs:v4'], 'KV не записан');
  const cached = JSON.parse(fakeData.kv['basket:pairs:v4']);
